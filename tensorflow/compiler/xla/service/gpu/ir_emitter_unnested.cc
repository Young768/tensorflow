...
Status IrEmitterUnnested::EmitConvolutionThunk(mlir::Operation* op) {
  ...
  if (auto conv = dyn_cast<ConvForwardOp>(op)) {
    ...
    auto alpha = conv.getAlpha().convertToDouble();
    VLOG(0)<<"Debug ###############$$$$$$$$$$$$$$$$$$$$ "<<alpha;
    if (alpha != 0.0) descriptor.backend_config.set_leakyrelu_alpha(alpha);
  } else {
    return InternalError("Unexpected operation");
  }
  VLOG(0)<<"Debug from EmitConvolutionThunk "<<descriptor.backend_config.leakyrelu_alpha();
